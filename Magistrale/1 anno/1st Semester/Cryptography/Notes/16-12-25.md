...
one pass unilateral key establishment use previos schema adding session key rand number 
same for the other schema but adding session key (unilateral)

mutual -> both will generate a different session key -> they need to agree on a function to put them togheter 

diffie hellman key agreement protocol -> genearate a new secret key without preasgreed shared key 

generate prime number p -> generator a of p number elevated by all the numbers between 1 and p-1 mod p > result will be all the number between 1 and p-1 

given a prime numebr there is always a generator -> discrete log -> a elevated to discrete log gives back b

$$
a ^ i \mod p = b
$$

fidning it is np-hard 

p such that p-1 has at least 1 big prime factor 

diffie hellman attack -> go in the middle and exchagne a common key between 2 -> tahn we can decrypt everything
