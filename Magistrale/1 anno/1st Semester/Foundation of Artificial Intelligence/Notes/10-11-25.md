...
if them margin > 1 -> 0 otherwise the new margin

hinge loss -> still a convex function 

lower C -> gives up more easily for missclassification  

svm with non linearly seaparable data 

if we perform a non linear trasformation into a n higher dimensional space it may become linearly separable 

we just need a way of computing the dot product of two 

no metter how i pick a finite subset of x and contruct matrix M  of k(x_i,x_j) -> allow for 0 but each $x_i\subseteq X$

dot product has this propiety

mercer kernel -> given K a positive definite function than there a exists a vector space Y and function $X \to Y$ such that :
$$
K(x_1,x_2) = \phi(x_1)\cdot \phi(x_2)
$$
we can sobstitute the dot product with the function K 

expected crossvalidation error is less than the num of support vector / number of training samples 

if larger than we are probably overfitting 